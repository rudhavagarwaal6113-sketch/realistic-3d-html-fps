...
const controls=new PointerLockControls(camera,document.body); scene.add(camera); const clock=new THREE.Clock(),raycaster=new THREE.Raycaster(); const keys={}; let locked=false,health=100,ammo=30,reserve=120,reloading=false,aiming=false,lastShot=0,recoil=0,bob=0,vy=0,grounded=true,crouch=false,kills=0,score=0,damageFlash=0,reloadStart=0;
...
const barrel=new THREE.Mesh(new THREE.CylinderGeometry(.036,.042,.72,16),metalMat);barrel.rotation.x=Math.PI/2;barrel.position.z=-.7;weapon.add(barrel);
const stock=new THREE.Mesh(new THREE.BoxGeometry(.18,.25,.48),gunMat);stock.position.z=.57;weapon.add(stock);
const optic=new THREE.Group();optic.position.set(0,.145,-.18);weapon.add(optic);const scopeBody=new THREE.Mesh(new THREE.CylinderGeometry(.055,.065,.28,16),gunMat);scopeBody.rotation.x=Math.PI/2;optic.add(scopeBody);const scopeGlass=new THREE.Mesh(new THREE.CircleGeometry(.039,24),new THREE.MeshBasicMaterial({color:0x55d7e8,transparent:true,opacity:.6}));scopeGlass.rotation.y=Math.PI;scopeGlass.position.z=-.142;optic.add(scopeGlass);
const frontSight=new THREE.Mesh(new THREE.BoxGeometry(.018,.08,.018),gunMat);frontSight.position.set(0,.11,-.38);weapon.add(frontSight);
const muzzle=new THREE.PointLight(0xffb35c,0,8);muzzle.position.z=-1.05;weapon.add(muzzle);
const arms=new THREE.Group();weaponRig.add(arms);
const forearmL=new THREE.Mesh(new THREE.CapsuleGeometry(.105,.48,5,10),sleeveMat);forearmL.position.set(-.23,-.02,-.04);forearmL.rotation.set(-.3,.05,.28);arms.add(forearmL);const handL=new THREE.Mesh(new THREE.SphereGeometry(.105,14,10),skinMat);handL.position.set(-.07,-.055,-.37);arms.add(handL);
const forearmR=new THREE.Mesh(new THREE.CapsuleGeometry(.105,.48,5,10),sleeveMat);forearmR.position.set(.21,-.01,.02);forearmR.rotation.set(-.3,-.05,-.3);arms.add(forearmR);const handR=new THREE.Mesh(new THREE.SphereGeometry(.105,14,10),skinMat);handR.position.set(.075,-.055,-.34);arms.add(handR);
const targets=[];
...
function reload(){if(reloading||ammo===30||reserve<=0)return;reloading=true;reloadStart=performance.now();updateUI();setTimeout(()=>{const n=Math.min(30-ammo,reserve);ammo+=n;reserve-=n;reloading=false;updateUI()},1050)}
...
function animate(){requestAnimationFrame(animate);const dt=Math.min(clock.getDelta(),.033),t=clock.elapsedTime;move(dt);updateTargets(t);recoil=THREE.MathUtils.lerp(recoil,0,dt*12);const moving=keys.KeyW||keys.KeyA||keys.KeyS||keys.KeyD;const sprinting=moving&&(keys.ShiftLeft||keys.ShiftRight)&&!crouch&&!aiming;const reloadT=reloading?Math.min(1,(performance.now()-reloadStart)/1050):0;const reloadArc=reloading?Math.sin(reloadT*Math.PI):0;const bobX=moving?Math.sin(bob)*.012:Math.sin(t*1.5)*.003;const bobY=moving?Math.abs(Math.cos(bob))*.014:Math.sin(t*1.7)*.002;const tx=aiming?0:.02,ty=aiming?-.105:-.16,tz=aiming?-.39:-.48;weaponRig.position.x=THREE.MathUtils.lerp(weaponRig.position.x,tx+(sprinting?.055:0)+bobX,dt*14);weaponRig.position.y=THREE.MathUtils.lerp(weaponRig.position.y,ty+(sprinting?.055:0)-reloadArc*.13+bobY,dt*14);weaponRig.position.z=THREE.MathUtils.lerp(weaponRig.position.z,tz+reloadArc*.16,dt*14);weaponRig.rotation.x=THREE.MathUtils.lerp(weaponRig.rotation.x,-recoil+(sprinting?Math.sin(bob)*.045:0)+reloadArc*.45,dt*15);weaponRig.rotation.y=THREE.MathUtils.lerp(weaponRig.rotation.y,aiming?0:(sprinting?Math.sin(bob)*.035:0),dt*15);weaponRig.rotation.z=THREE.MathUtils.lerp(weaponRig.rotation.z,sprinting?Math.sin(bob)*.04:0,dt*15);arms.rotation.x=THREE.MathUtils.lerp(arms.rotation.x,reloadArc*.7,dt*14);arms.rotation.z=THREE.MathUtils.lerp(arms.rotation.z,(sprinting?-.1:0)+reloadArc*.25,dt*14);camera.fov=THREE.MathUtils.lerp(camera.fov,aiming?58:sprinting?77:70,dt*12);camera.updateProjectionMatrix();if(damageFlash>0){damageFlash=Math.max(0,damageFlash-dt);document.body.style.setProperty('--damage',damageFlash*.28)}else document.body.style.setProperty('--damage','0');renderer.render(scene,camera)}
