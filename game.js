...
function animate(){requestAnimationFrame(animate);const dt=Math.min(clock.getDelta(),.033),t=clock.elapsedTime;move(dt);updateTargets(t);recoil=THREE.MathUtils.lerp(recoil,0,dt*11);const moving=keys.KeyW||keys.KeyA||keys.KeyS||keys.KeyD;const sprinting=moving&&(keys.ShiftLeft||keys.ShiftRight)&&!crouch&&!aiming;const sway=moving?Math.sin(bob)*.008:0;const sprintBob=sprinting?Math.sin(bob*1.35)*.018:0;const reloadT=reloading?Math.min(1,(performance.now()-reloadStart)/1050):0;const reloadArc=reloading?Math.sin(reloadT*Math.PI):0;const baseX=aiming?.18:.36,baseY=aiming?-.28:-.35,baseZ=aiming?-.52:-.62;weapon.position.x=THREE.MathUtils.lerp(weapon.position.x,baseX+(sprinting?.06:0),dt*10)+sway;weapon.position.y=THREE.MathUtils.lerp(weapon.position.y,baseY+(sprinting?.045:0)-reloadArc*.22,dt*10)+sprintBob;weapon.position.z=THREE.MathUtils.lerp(weapon.position.z,baseZ+reloadArc*.16,dt*10);weapon.rotation.x=-.04-recoil+(sprinting?Math.sin(bob)*.035:0);weapon.rotation.y=aiming?-.01:0;arms.rotation.z=THREE.MathUtils.lerp(arms.rotation.z,(sprinting?-.12:0)+reloadArc*.35,dt*12);arms.rotation.x=THREE.MathUtils.lerp(arms.rotation.x,reloadArc*.5,dt*12);camera.fov=THREE.MathUtils.lerp(camera.fov,aiming?58:sprinting?76:70,dt*12);camera.updateProjectionMatrix();if(damageFlash>0){damageFlash=Math.max(0,damageFlash-dt);document.body.style.setProperty('--damage',damageFlash*.28)}else document.body.style.setProperty('--damage','0');renderer.render(scene,camera)}
