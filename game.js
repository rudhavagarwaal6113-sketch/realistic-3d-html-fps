...
function showHit(text,cls=''){const e=document.getElementById('hittext');e.textContent=text;e.className='show '+cls;setTimeout(()=>e.className='',420)}
function updateTargets(t){for(const g of targets){if(!g.userData.alive)continue;g.position.x=g.userData.baseX+Math.sin(t*g.userData.speed+g.userData.phase)*3.2;g.rotation.y=Math.sin(t*.7+g.userData.phase)*.18;const dist=camera.position.distanceTo(g.position);if(dist<28&&Math.random()<.0018){damageFlash=.7;health=Math.max(0,health-8);showHit('TAKING FIRE','damage');updateUI()}}}
function updateUI(){document.getElementById('ammo').textContent=ammo;document.getElementById('reserve').textContent='/ '+reserve;document.getElementById('health').textContent=Math.round(health);document.getElementById('healthBar').style.width=health+'%';document.getElementById('weaponState').textContent=reloading?'RELOADING':aiming?'ADS':'READY';document.getElementById('score').textContent=String(score).padStart(5,'0');document.getElementById('kills').textContent=String(kills).padStart(2,'0')}
