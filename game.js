...
function updateTargets(t){for(const g of targets){if(!g.userData.alive)continue;g.position.x=g.userData.baseX+Math.sin(t*g.userData.speed+g.userData.phase)*3.2;g.rotation.y=Math.sin(t*.7+g.userData.phase)*.18;const eye=new THREE.Vector3();camera.getWorldPosition(eye);const enemyEye=new THREE.Vector3();g.userData.head.getWorldPosition(enemyEye);const toPlayer=eye.clone().sub(enemyEye);const dist=toPlayer.length();if(dist<28){const dir=toPlayer.clone().normalize();const enemyForward=new THREE.Vector3(0,0,1).applyQuaternion(g.quaternion);enemyForward.y=0;dir.y=0;dir.normalize();enemyForward.normalize();const facing=enemyForward.dot(dir);if(facing>.82){const los=new THREE.Raycaster(enemyEye,dir,0,dist);const blockers=los.intersectObjects(scene.children,true).filter(h=>h.object!==g.userData.body&&h.object!==g.userData.head&&h.object!==weapon&&h.object!==camera);if(!blockers.length&&Math.random()<.00055){damageFlash=.45;health=Math.max(0,health-8);showHit('TAKING FIRE','damage');updateUI()}}}}}}}
