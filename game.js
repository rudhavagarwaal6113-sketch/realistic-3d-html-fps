...
const optic=new THREE.Group(); optic.position.set(0,.105,-.18); weapon.add(optic); const scopeBody=new THREE.Mesh(new THREE.CylinderGeometry(.055,.065,.28,16),mat(0x151719,.24,.85)); scopeBody.rotation.x=Math.PI/2; optic.add(scopeBody); const scopeGlass=new THREE.Mesh(new THREE.CircleGeometry(.038,24),new THREE.MeshBasicMaterial({color:0x4ec7d9,transparent:true,opacity:.55})); scopeGlass.rotation.y=Math.PI; scopeGlass.position.z=-.142; optic.add(scopeGlass); const frontSight=new THREE.Mesh(new THREE.BoxGeometry(.018,.08,.018),mat(0x050607,.2,.9)); frontSight.position.set(0,.11,-.38); weapon.add(frontSight);
const muzzle=new THREE.PointLight(0xffb35c,0,8); muzzle.position.z=-1.05; weapon.add(muzzle);
// Procedural first-person arms: lightweight now, ready to be replaced by a rigged model later.
const arms=new THREE.Group(); weapon.add(arms);
const sleeveMat=mat(0x20272a,.85,.02),skinMat=mat(0xa86f51,.92,0);
const forearmL=new THREE.Mesh(new THREE.CapsuleGeometry(.105,.48,5,10),sleeveMat); forearmL.position.set(-.25,-.12,-.2); forearmL.rotation.z=.18; arms.add(forearmL);
const forearmR=new THREE.Mesh(new THREE.CapsuleGeometry(.105,.48,5,10),skinMat); forearmR.position.set(.19,-.1,-.28); forearmR.rotation.z=-.28; arms.add(forearmR);
const glove=new THREE.Mesh(new THREE.BoxGeometry(.17,.14,.22),skinMat); glove.position.set(-.08,-.12,-.43); arms.add(glove);
...
function animate(){requestAnimationFrame(animate);const dt=Math.min(clock.getDelta(),.033),t=clock.elapsedTime;move(dt);updateTargets(t);recoil=THREE.MathUtils.lerp(recoil,0,dt*12);const moving=keys.KeyW||keys.KeyA||keys.KeyS||keys.KeyD;const sprinting=moving&&(keys.ShiftLeft||keys.ShiftRight)&&!crouch&&!aiming;const runBob=sprinting?Math.sin(bob*1.45):Math.sin(bob);const sideBob=moving?Math.cos(bob)*.012:0;const reloadT=reloading?Math.min(1,(performance.now()-reloadStart)/1050):0;const reloadArc=reloading?Math.sin(reloadT*Math.PI):0;const targetX=aiming?0:.36,targetY=aiming?-.20:-.35,targetZ=aiming?-.66:-.62;weapon.position.x=THREE.MathUtils.lerp(weapon.position.x,targetX+(sprinting?.075:0)+sideBob,dt*14);weapon.position.y=THREE.MathUtils.lerp(weapon.position.y,targetY+(sprinting?.06:0)-reloadArc*.28+(!moving&&!aiming?Math.sin(t*1.7)*.004:0),dt*14);weapon.position.z=THREE.MathUtils.lerp(weapon.position.z,targetZ+reloadArc*.20,dt*14);weapon.rotation.x=THREE.MathUtils.lerp(weapon.rotation.x,-.035-recoil+(sprinting?runBob*.045:0)+reloadArc*.65,dt*16);weapon.rotation.y=THREE.MathUtils.lerp(weapon.rotation.y,aiming?0:(sprinting?runBob*.035:0),dt*14);weapon.rotation.z=THREE.MathUtils.lerp(weapon.rotation.z,(sprinting?Math.sin(bob*1.45)*.045:0)+(reloadArc?Math.sin(reloadT*Math.PI*2)*.18:0),dt*14);arms.rotation.x=THREE.MathUtils.lerp(arms.rotation.x,reloadArc*.9+(sprinting?runBob*.08:0),dt*14);arms.rotation.z=THREE.MathUtils.lerp(arms.rotation.z,(sprinting?-.16:0)+reloadArc*.45,dt*14);camera.fov=THREE.MathUtils.lerp(camera.fov,aiming?58:sprinting?77:70,dt*12);camera.updateProjectionMatrix();if(damageFlash>0){damageFlash=Math.max(0,damageFlash-dt);document.body.style.setProperty('--damage',damageFlash*.28)}else document.body.style.setProperty('--damage','0');renderer.render(scene,camera)}
